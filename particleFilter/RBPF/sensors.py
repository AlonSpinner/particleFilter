...
class laser2:
    ...
    def __init__(self, angles):
        ...
        self.beta = np.radians(10) #beam anglar width in radians
        self.zmax = 10.0 #meters
    ...
    def inverse_measurement_model(self, x : pose2, z : np.ndarray, m : gridmap2): #returns p(m|xt,zt)
        ...
        for c in cells:
            ...
            if r > min(self.zmax,z[k]+self.alpha/2) or abs(self.angles[k]-phi) > self.beta/2: #distance and angle don't fit beam
                update.append('prior')
            elif z[k] < self.zmax and abs(r-z[k]) < self.alpha/2:
                update.append('occ')
            else:
                update.append('free')
    ...
    def questionCells(self, x : pose2, m : gridmap2):
        ...
        c = m.c2d(tuple(x.t()))
        aI = np.ceil((self.zmax + self.alpha/2) / m.resY)
        aJ = np.ceil((self.zmax + self.alpha/2) / m.resX)
            
        i_bracket = np.clip(np.array([c[0] - aI,c[0] + aI]),0,m.height-1).astype(int)
        j_bracket = np.clip(np.array([c[1] - aJ,c[1] + aJ]),0,m.width-1).astype(int)
    ...
    def questionCells(self, x : pose2, m: gridmap2, a : float, z : float):
