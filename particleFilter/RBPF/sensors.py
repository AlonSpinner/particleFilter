...
class beamer():
    def __init__(self, angles = np.linspace(-np.pi,np.pi,180), zmax = 5.0, beta = np.radians(10), alpha = 0.2, res = 0.1):
        ...
        self.localraymap : list[beam] = self.localgrid2raymap()

    def localgrid2raymap(self): #list of rays
    #create localraymap grid - should be called only once when initalizing
        ...
        for a in self.angles:
            ...
            raymap.append(beam(a,t,z,idx))
            return raymap

    def localGrid(self):
        #called by localgrid2raymap()
        #-------------
        #|         k |
        #|      ---->|
        #|           |
...
class laser:
    def __init__(self, angles = np.linspace(-np.pi,np.pi,180), zmax = 5.0):
        self.pose  = pose2(0,0,0)
